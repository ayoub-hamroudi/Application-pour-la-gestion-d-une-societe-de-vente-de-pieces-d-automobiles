...
class Login extends JFrame implements ActionListener
{
	...
	public void actionPerformed(ActionEvent ae)
	{
		if(ae.getSource() == bOk)
		{
			if(tUser.getText().equals(""))		// si l'utilisateur laisse le champ username vide
			{
				JOptionPane.showMessageDialog(this, "Please provide Username to login");
				tUser.requestFocus();
			}
			else if(pPass.getText().equals(""))	// si l'utilisateur laisse le champ password vide
			{
				JOptionPane.showMessageDialog(this, "Please provide Password to login");
				pPass.requestFocus();
			}
			else								// Si l'utilisateur donne le username et le password
			{
				String User, Pass;
				Boolean check = true;
				User = tUser.getText();
				...
				if(check == true)
				{
					try
					{
						...
						if(rs.next())										//  usrename correct
						{
							if((Pass.equals(rs.getString("Password"))))		// password correct
							{
								new Welcome();
								dispose();
								verify = true;
